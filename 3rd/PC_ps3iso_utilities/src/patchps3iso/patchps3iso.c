...
*/
//	2014.3.5	Mod by KiD
//	Support replace file in the ISO.
...
static u8 *replace_sectors = NULL;
...
static int replace_file(const char* name, u32 lba, u32 len, struct iso_directory_record * idr)
{
	struct stat s;
	if(stat(name, &s)<0) {return -1;}
	
	u32 llen = (len + 2047) & ~2047;
	if(s.st_size > llen) {printf("  Error: %s size %I64d > %u.\n", name, s.st_size, llen); return -1;}


		FILE * fp2 = fopen(name, "rb");
		if(!fp2) {return -1;}
		
		u32 orglen = s.st_size;
		...
		while(s.st_size > 0) {
			...
			//if(fsize < 0x40000) memset(replace_sectors, 0, 0x40000);
			...
			/*if(is_file_split) {
				...
				if(fread((void *) replace_sectors, 1, (int) fsize, fp2) != fsize) {
					fclose(fp2); return -668;
				}
			//}
	
			lsize = (fsize + 2047) & ~2047;
			//printf("write_split  %d %d\n", lba, lsize/2048);
	
			//int ret = write_split(fp, lba, sectors, lsize/2048, 1);
			
			u64 file_pos = ((u64) lba * 2048ULL);
			int ret = write_split2(file_pos, replace_sectors, fsize);
	
			if(ret < 0) {
				if(verbose) printf("\n");
				fclose(fp2); return ret;
	
			}
	
			lba += lsize/2048;
	
			//if(!verbose) printf("\rPercent done: %u%% \r", (u32) (((u64) lba) * 100ULL / ((u64) toc)));
			//printf("flba %i\n", flba * 2048);
	...
	if(orglen < 1024ULL) 
		{if(verbose) printf("  replace %s LBA %u size %u Bytes\n", name, lba, (u32) orglen);}
	else if(s.st_size < 0x100000LL) 
		{if(verbose) printf("  replace %s LBA %u size %u KB\n", name, lba, (u32) (orglen/1024));}
	else
		{if(verbose) printf("  replace %s LBA %u size %u MB\n", name, lba, (u32) (orglen/0x100000LL));}
	set733((void *) idr->size, orglen);
...
int main(int argc, const char* argv[])
{
    ...
    if(argc > 1 && (!strcmp(argv[1], "/?") || !strcmp(argv[1], "--help"))) {
        ...
        printf("\nPATCHPS3ISO (c) 2013, Estwald (Hermes)\tMod by KiD\n\n");
        ...
        printf("%s", "Usage:\n\n"
		       ...
		       "    patchps3iso -r <replace_dir> <ISO file> <version>  -> replace file in ISO\n");
    ...
    replace_path[0] = 0;
    if (argc - a < 3 && argc >= 2 && !strcmp(argv[a+1], "-r"))
	{
		printf("Enter replace dir:\n");
		if(fgets(replace_path, 0x420, stdin)==0) {
			printf("Error replace dir!\n\nPress ENTER key to exit\n"); get_input_char();return -1;
		}
		replace = 1; 
	}
	else if (argc - a >= 3 && !strcmp(argv[a+1], "-r"))
	{
		strcpy(replace_path, argv[2 + a]); 
		a+=2; 
		replace = 1; 
	}
	...
	{
		if(replace_path[0] == 0) {
			 printf("Error: dir don't exists!\n\nPress ENTER key to exit\n"); get_input_char();return -1;
		}
		fixpath(replace_path); 
		
        if(stat(replace_path, &s)<0) {
            printf("Error: dir don't exists!\n\nPress ENTER key to exit\n"); get_input_char();return -1;
		}
		replace_sectors = malloc(128 * 2048 + 2048);
		printf("replace mode: %s\n", replace_path);
	}

    if(argc == 1 || (replace && argc < 4)) {
    ...
    while(p < size0) {
		...
		{
			char name[0x420];
			strcpy(name, replace_path);
			strcat(name, string2);
			update_idr = replace_file(name, file_lba, file_size, idr);
			goto next_file;
		}
		...
		if (update_idr >= 0)
		{
			...
			if(fwrite((void *) (sectors2), 1, 2048, fp) != 2048) {
				printf("Error!: reading directory_record sector\n\n");
				goto err;
			}
		}
